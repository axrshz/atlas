...
func NewAgent(client *openai.Client, getUserMessage func() (string, bool), tools []ToolDefinition, config Config, sessionManager *SessionManager) *Agent {
	return &Agent{
		client:         client,
...
func (a *Agent) loadOrCreateSession() error {
	...
	session, err := a.sessionManager.GetLatestSession()
	if err == nil {
		a.session = session
